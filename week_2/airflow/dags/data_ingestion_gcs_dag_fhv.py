...
def upload_to_gcs(bucket, object_name, local_file):
    ...
    :return: 
    """
    # WORKAROUND to prevent timeout for files > 6 MB on 888 Kbps upload speed.
...
with DAG(
    ...
    format_to_parquet = PythonOperator(
        ...
        op_kwargs={
            "src_file": f"{path_to_local_home}/{DATASET_TEMPLATE}"
        },
    )

    # TODO: Homework - research and try XCOM to communicate output values between 2 task/operators
    local_to_gcs_task = PythonOperator(
        task_id="local_to_gcs_task",
        python_callable=upload_to_gcs,
        op_kwargs={
            "bucket": BUCKET,
            "object_name": f"raw/{PARQUET_TEMPLATE}",
            "local_file": f"{path_to_local_home}/{PARQUET_TEMPLATE}"
        },
    )

    clean_dir = BashOperator(
        task_id="clean_dir",
        bash_command=f"rm {path_to_local_home}/{DATASET_TEMPLATE} {path_to_local_home}/{PARQUET_TEMPLATE}"
    )


    download_dataset_task >> format_to_parquet >> local_to_gcs_task >> clean_dir